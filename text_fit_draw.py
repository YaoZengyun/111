...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        result_bytes = draw_text_auto(
            image_source=params.get("image_source", "base.png"),
            top_left=tuple(params.get("top_left", [0, 0])),
            bottom_right=tuple(params.get("bottom_right", [800, 600])),
            text=params.get("text", ""),
            color=tuple(params.get("color", [0, 0, 0])),
            max_font_height=params.get("max_font_height"),
            font_path=params.get("font_path"),
            align=params.get("align", "center"),
            valign=params.get("valign", "middle"),
            line_spacing=params.get("line_spacing", 0.15),
            bracket_color=tuple(params.get("bracket_color", [128, 0, 128])),
            image_overlay=params.get("image_overlay")
        )
        
        # 输出base64编码的结果
        print(base64.b64encode(result_bytes).decode())
